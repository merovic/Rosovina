...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        TabbySDK.shared.setup(withApiKey: "pk_test_10d6975e-c7b5-4c24-b2a9-e9d27b5cd4ed")
        
        SCSDKSnapKit.initialize()
        
        // Log a custom event
        SCSDKSnapAPI.fetchUserData(withQuery: "{me{bitmoji{avatar}}}") { (response, error) in
            if let error = error {
